...
var player0Color = 'rgb(211, 211, 211)';
...
function colorMatchCheck(one,two,three,four){
  return (one===two && one===three && one===four && one !== player0Color && one !== undefined);
}
...
// Game End
function gameEnd(winningPlayer, winningPlayerColor) {
  for (var col = 0; col < 7; col++) {
    for (var row = 0; row < 7; row++) {
      $('h1').html(winningPlayer+" has won!").css("fontSize", "50px")
      $('h1').css('background-color', winningPlayerColor)
      $('h2').html("Refresh your browser to play again!")
      $('h3').fadeOut('fast');
    }
  }
}
